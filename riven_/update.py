...
class RivenUpdate(Update):
    ...
    def update_check(self, process_name):
        ...
        try:
            ...
            self.logger.info(
                f"Currently installed [v{current_version}] for {process_name}"
            )
            ...
            self.logger.info(
                f"Latest available version [v{latest_version}] for {process_name}"
            )
    ...
    def start_process(self, process_name, config_dir=None, suppress_logging=False):
        if process_name == "riven_frontend":
            ...
            while not os.path.exists(directory):
                self.logger.info(
                    f"Waiting for symlink directory {directory} to become available before starting {process_name}"
                )
                time.sleep(10)
