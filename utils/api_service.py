...
class APIState:
    ...
    def poll_update_status(self, process_name):
        ...
        if process is None:
            self.logger.warning(f"Process {process_name} not found, setting status as unknown.")
            return False
        is_running = process.poll() is None
        self.logger.debug(f"Poll result for {process_name}: {'running' if is_running else 'stopped'}")
        return is_running
...
def create_app(riven_updater, zilean_updater, zurg_updater, process_handler, logger):
    ...
    origins = (
        [origin_from_env] if origin_from_env else ["http://localhost", "http://localhost:8000"]
    )
    ...
    async def stop_service(
        ...
        logger.info(f"Received stop-service request for {request.process_name}, Request ID: {id(request)}")
        ...
        if process_name in api_state.shutdown_in_progress:
            ...
            return {"status": "Shutdown already in progress", "process_name": process_name}
