...
def zurg_setup(process_handler=None):
    ...
    def update_token(file_path, token):
        ...
        with open(file_path, "w") as file:
            for line in lines:
                if line.strip().startswith("token:") or line.strip().startswith(
                    "# token:"
                ):
    ...
    def update_port(file_path, port):
        ...
        with open(file_path, "w") as file:
            for line in lines:
                if line.strip().startswith("port:") or line.strip().startswith(
                    "# port:"
                ):
            ...
            for line in lines:
                if zurguser and zurgpass:
                    if line.strip().startswith("username:") or line.strip().startswith(
                        ...
                        "password:"
                    ) or line.strip().startswith("# password:"):
        ...
        if os.path.exists(zurg_binary_path) and not ZURGVERSION:
            try:
                result = subprocess.run(
                    [zurg_binary_path, "version"], capture_output=True, text=True
                )
    ...
    def setup_zurg_instance(config_dir, token, key_type):
        try:
            ...
            if os.path.exists(zurg_app_override):
                logger.debug(
                    f"Copying Zurg app from override: {zurg_app_override} to {zurg_executable_path}"
                )
                ...
                not os.path.exists(zurg_executable_path)
                or not os.environ.get("ZURG_CURRENT_VERSION")
                or ZURGVERSION
            ):
                logger.debug(
                    f"Copying Zurg app from base: {zurg_app_base} to {zurg_executable_path}"
                )
                shutil.copy(zurg_app_base, zurg_executable_path)
            elif os.environ.get(
                "ZURG_CURRENT_VERSION"
            ) == ZURGVERSION and os.path.exists(zurg_executable_path):
            ...
            if os.path.exists(zurg_config_override):
                logger.debug(
                    f"Copying Zurg config from override: {zurg_config_override} to {config_file_path}"
                )
                shutil.copy(zurg_config_override, config_file_path)
            elif not os.path.exists(config_file_path):
                logger.debug(
                    f"Copying Zurg config from base: {zurg_config_base} to {config_file_path}"
                )
            ...
            update_creds(
                config_file_path, ZURGUSER, ZURGPASS if ZURGUSER and ZURGPASS else None
            )
    ...
    try:
        ...
        if RDAPIKEY:
            rd_dir = "/zurg/RD/"
            logger.info(
                f"Setting up Zurg w/ RealDebrid instance in directory: {rd_dir}"
            )
        ...
        raise Exception(
            f"FileNotFoundError: The file was not found during zurg setup - {e}"
        )
    except PermissionError as e:
        raise Exception(
            f"PermissionError: Permission denied during file operation or subprocess execution for zerg setup - {e}"
        )
    except Exception as e:
